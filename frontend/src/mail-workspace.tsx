...
export function MailWorkspace({ base, boxes, boxesError, revision, canWrite, renderDetail, onAction, onClassify }: {
  ...
  function chooseMailbox(id: string) { setMailbox(id); setOffset(0); setSelected(""); setFilter("all"); setSearch(""); }
  return <section className="mail-workspace">
    ...
    <section className="mail-message-list" aria-label="邮件列表">
      <header><h2>{currentBox?.label || "全部邮箱"}</h2><small>{state.data ? `${state.data.total} 封邮件` : "正在加载…"}</small>
        <input aria-label="搜索邮件" placeholder="搜索主题、发件人或产品" value={search} onChange={(e) => { setSearch(e.target.value); setOffset(0); setSelected(""); }} />
        <select aria-label="邮件处理状态" value={filter} onChange={(e) => { setFilter(e.target.value); setOffset(0); setSelected(""); }}>
      ...
      <footer className="mail-list-pagination">
        <Button variant="ghost" disabled={offset === 0 || state.loading} onClick={() => { setOffset(Math.max(0, offset - 50)); setSelected(""); }}>上一页</Button>
        <span>{Math.floor(offset / 50) + 1} / {Math.max(1, Math.ceil((state.data?.total || 0) / 50))}</span>
        <Button variant="ghost" disabled={state.loading || !state.data || offset + 50 >= state.data.total} onClick={() => { setOffset(offset + 50); setSelected(""); }}>下一页</Button>
      </footer>
    </section>
