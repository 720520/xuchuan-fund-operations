...
当前是可运行的核心链路实现，不代表全部第一阶段业务已经验收。文档已记录归档扫描和托管适配，真实连续运行、完整持仓及收益口径的验收证据仍需核实，见[本地验收报告](docs/本地验收报告.md)。
...
- [文档入口](docs/README.md)：产品范围、本地验收、开发阶段及必要开发说明。

## 净值自动应收（2026-09-08）

已新增交易日应收、15:00 迟到、下个交易日跟进、托管补发留痕及材料到达确认。升级后重启应用，在“组织与权限 → 净值自动应收”核对并启用；首次默认关闭，跟进时间初始建议 09:00，可配置。当前日历覆盖上交所 2025–2026 年，超范围会提示更新。真实业务试运行尚待完成，见[开发阶段](docs/开发阶段.md)。
...
- 完整邮箱账号和客户端授权码；
- 从哪一天开始同步、仅收件箱或全部业务文件夹；全部业务文件夹会自动排除已发送、草稿、垃圾邮件和已删除；
...
原邮件采用 `readonly=True`、`BODY.PEEK[]` 收取，不设置已读、不移动、不删除。连接层只向同步代码开放安全读取操作，不提供删除、移动、复制、追加、标记、目录修改或发送接口。全部业务文件夹模式按“文件夹 + UIDVALIDITY + UID”断点续收；同一原件出现在多个文件夹时按 SHA-256 只归档一份。邮件先分类，只有净值/估值附件进入解析；明确风险、业务动作和安全提醒进入邮件待办，未知内容进入待分类。每轮每个邮箱最多收取 100 封，持续运行会分批完成历史导入。自动刷新 OAuth token、连续故障退避、更大邮件容量及规则细化仍需生产联调。
...
2026-09-08 本地验收：SQLite 后端 66 项通过、4 项并发测试跳过；PostgreSQL 回归 70 项全部通过；前端 3 项数字格式测试、类型检查和生产构建通过。浏览器走通自动应收设置、缺件检查、补发记录及原件匹配；原净值、冲突与反账通过回归。完整结果与限制见[本地验收报告](docs/本地验收报告.md)。
...
env PYTHONPATH=backend .venv/bin/python backend/tests/serve_qa.py
```
